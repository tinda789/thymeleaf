...
public class ProjectService {
    ...
    /**
     * Kiểm tra quyền tạo dự án trong workspace
     * @param workspace Workspace cần kiểm tra
     * @param user User đang thực hiện hành động
     * @throws RuntimeException nếu user không có quyền
     */
    public void checkProjectCreationPermission(Workspace workspace, User user) {
    ...
    // Bổ sung phương thức tạo dự án với kiểm tra quyền
    @Transactional
    public Project createProjectWithPermissionCheck(Long workspaceId, ProjectCreateDto dto, String username) {
        Workspace workspace = workspaceRepository.findById(workspaceId)
                .orElseThrow(() -> new RuntimeException("Workspace not found"));
        ...
        // Kiểm tra quyền
        checkProjectCreationPermission(workspace, user);
